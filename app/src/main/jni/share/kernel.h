...
#define     MAX_CANDIDATES          600                         //最大的候选项个数
...
#define     MAX_SYLLABLE_PER_INPUT  64                              //一次能够输入的音节数目
...
//词项类型
typedef struct tagUSERWORDITEM
{
    unsigned int      effective : 1;     //有效（没有被删除）
    unsigned int       ci_length : 6;     //词长度
    unsigned int       syllable_length : 6;      //音节长度
    ...
    //(见AddCi和InsertCiToCache)，其实准确地说，在装好输入法开始使用以后，使用度才是使用1次加1，而词库中的次品是不变的，
    //(用户词库除外)，最初的词库中的词频信息是利用原始语料库中的词频换算得到的(通常不是0，使用度的初始值才是0)，换算公
    //式见 AdjustWordFreqToHZFreq))
    SYLLABLE      syllable[USER_MAX_SYLLABLES];      //音节
    HZ       hz[USER_MAX_SYLLABLES];            //汉字
}USERWORDLIBITEM;
//字候选类型
