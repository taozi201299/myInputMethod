...
/*!
 * \brief 判断第一个音节参数所指示的声母集合是否包含第二个音节参数的声母。
 * \param   syllable                源音节
 * \param   checked_syllable        被检查音节
...
#define CHECK_FUZZY(fuzzy_bit, con_value)   ((fuzzy_mode & fuzzy_bit) && (checked_syllable.con == con_value))
    ...
    /**
     * 比较情况
     * 1 syllable.con checked_syllable.con 都为空  syllable.con == checked_syllable.con 比较韵母
     * 2 syllable.con checked_syllable.con 都不为空
     *    如果 syllable.con == checked_syllable.con
     *    如果 syllable.con != checked_syllable.con  需要考虑模糊查询
     * 3 syllable.con 为空 checked_syllable.con 不为空
     * 4 syllable.con 不为空  checked_syllable.con 为空
     */
    ...
    if(syllable.con == CON_NULL && checked_syllable.con != CON_NULL){
        return  0;
    }
    //被检查的声母为空则为不包含
...
/*!
 * \brief 判断是否是双拼方案设这引起的模糊(见sp_fuzzy_syllables定义处注释)
 * \param syllable                源音节
 * \param checked_syllable        被检查音节
 * \return
 *      1           包含
...
/*!
 * \brief  判断第一个音节参数所指示的韵母集合是否包含第二个音节参数的韵母。
 * \param  syllable                源音节
 * \param  checked_syllable        被检查音节
 * \param  fuzzy_mode              模糊设置
 * \return
...
/*!
 * \brief 不使用音调的比较，判断第一个音节参数所指示的音节集合是否包含第二个音节参数。
...
/*!
 * \brief 进行音调包含的比较，判断第一个音节参数所指示的音调集合是否包含第二个音节参数的音调。
 * \param  syllable                源音节
 * \param  checked_syllable        被检查音节
 * \return
 *      1           包含
...
/*!
 * \brief 使用带有音调的比较，判断第一个音节参数所指示的音节集合是否包含第二个音节参数。
 *  当源音节只有声母时，与相同声母的任何音节都是相同的。
 * \param  syllable                源音节
 * \param  checked_syllable        被检查音节
 * \param  fuzzy_mode              模糊设置
...
/*!
 * \brief 获得拼音的音节标识。当前拼音作为完整的拼音进行检索，找出音节表达。
 ...
 * \param  pin_yin                 拼音串
 * \param  syllable                音节
 * \param  *syllable_str_length    音节使用的拼音长度，可以为0
...
int GetSyllable(const TCHAR *pin_yin, SYLLABLE *syllable, int *syllable_str_length, int fuzzy_mode/*, int *correct_flag, int *separator_flag*/)
{
    ...
    //二分法查找拼音
    low = 0;
    hi = default_resource->syllable_map_items - 1;
    while(low <= hi)
    {
        mid = (low + hi) / 2;
        ...
        //由于音调为数字，所以比较时不会产生错误。
        if (py_length != default_resource->syllable_map[mid].pin_yin_length)
            ret = _tcscmp(py, default_resource->syllable_map[mid].pin_yin);
        else
            ret = _tcsncmp(py, default_resource->syllable_map[mid].pin_yin, py_length);
    ...
    if (default_resource->syllable_map[mid].fuzzy_flag && !(default_resource->syllable_map[mid].fuzzy_flag & fuzzy_mode))
        return 0;

    //如果该音需要在纠正的情况下才是有效的，则需要判断纠正标志是否设置
    //if (default_resource->syllable_map[mid].correct_flag && !(default_resource->syllable_map[mid].correct_flag & pim_config->correct_option))
    //  return 0;

    //存储结果
    syllable->con  = default_resource->syllable_map[mid].con;
    syllable->vow  = default_resource->syllable_map[mid].vow;
    syllable->tone = TONE_0;                //没有音调标识

    //if (correct_flag)
    //  *correct_flag = default_resource->syllable_map[mid].correct_flag;

    //if (separator_flag)
    //  *separator_flag = has_separator;

    if (has_tone)
        syllable->tone = GetTone(py + default_resource->syllable_map[mid].pin_yin_length);

    if (syllable_str_length)
    {
        *syllable_str_length = default_resource->syllable_map[mid].pin_yin_length;
...
int GetSPSyllable(const TCHAR *pin_yin, SYLLABLE *syllable, int *syllable_str_length, int fuzzy_mode)
{
    ...
    //找到第一个最大匹配项的位置
    for (i = 0; i < default_resource->syllable_map_items; i++)
    {
        if (!_tcscmp(py, default_resource->syllable_map[i].sp_pin_yin) &&
            max_length < default_resource->syllable_map[i].pin_yin_length)
        {
            max_length = default_resource->syllable_map[i].pin_yin_length;
            index = i;
        }
    ...
    if (sp_fuzzy_map_items < MAX_SYLLABLE_PER_INPUT)
    {
        ...
        for (i = 0; i < default_resource->syllable_map_items; i++)
        {
                ...
                max_length == default_resource->syllable_map[i].pin_yin_length)
            {
                ...
                {
                    sp_fuzzy_map[sp_fuzzy_map_items].sp_fuzzy_syllables[count].con = default_resource->syllable_map[i].con;
                    sp_fuzzy_map[sp_fuzzy_map_items].sp_fuzzy_syllables[count].vow = default_resource->syllable_map[i].vow;
        ...
        for (i = 0; i < sp_fuzzy_map[sp_fuzzy_map_items].sp_fuzzy_syllable_count; i++)
        {
            ...
            if (default_resource->syllable_map[index].fuzzy_flag && !(default_resource->syllable_map[index].fuzzy_flag & fuzzy_mode))
            {
    ...
    //存储结果
    syllable->con  = default_resource->syllable_map[i].con;
    syllable->vow  = default_resource->syllable_map[i].vow;
    syllable->tone = TONE_0;            //没有音调标识

    syllable_length = (int)_tcslen(default_resource->syllable_map[i].sp_pin_yin);
    if (has_tone)
...
int GetLegalPinYinLength(const TCHAR *pin_yin)
{
    return (int)_tcslen(pin_yin);
}

/*!
 * \brief 用递归的方式正向解析拼音串。
 * \param  pin_yin             拼音串
 * \param  *syllables          音节存储数组
 * \param  array_length        音节数组长度
 * \param  fuzzy_mode          模糊设置
 * \return
 *      0：没有解析出正确的结果
 *      > 0：解析出的音节数目
...
static int ProcessParse(const TCHAR *pin_yin, SYLLABLE *syllables, int array_length, int fuzzy_mode/*, int *correct_flag, int *separator_flag*/)
{
    ...
    memset(cur_py, 0, sizeof(cur_py));
    memset(new_pin_yin, 0, sizeof(new_pin_yin));
    ...
    */
    ...
    py_length = GetLegalPinYinLength(pin_yin);
...
/// 对于手机输入法无效
    ...
    for (i = min(MAX_PINYIN_LENGTH, py_length); i > 0; i--)
    {
        ...
        //复制第一个可能的音节
        CopyPartString(cur_py, new_pin_yin, py_length - syllable_string_length);
 ...
 *      0：非法
 */
static int LegalPinYin(TCHAR *pin_yin)
{
    int i, nlen = 0;

    if (pin_yin)
    	{
    		nlen = GetLegalPinYinLength(pin_yin);
    	}
	else
	{
	        return 0;
    ...
    for (i = 1; pin_yin[i] && i < nlen; i++)
    {
        ...
        case 'u':
            switch(pin_yin[i - 1])
            {
            case 'a':   case 'e':   case 'v':   case 'u':   case '\'':
                pin_yin[i] = 'w';
            }
        ...
        case 'i':
            ...
            case 'g':   case 'i':   case 'o':   case 'v':   case 'w':   case '\'':
                pin_yin[i] = 'y';
                break;
...
/*!
 * \brief 解析拼音串到音节数组。采用正向最大匹配的方式进行。
 * \param   pin_yin             拼音串（可能包含音节分隔符）
 * \param   *syllable_array     解析后的音节数组，以0为标识
 * \param   syllable_length     音节数组长度
 * \param   fuzzy_mode          模糊音设置
 * \return
 *      音节的数目，等于0的时候一般为错误输入
 */
int ParsePinYinString(TCHAR *pin_yin, SYLLABLE *syllables, int array_length, int fuzzy_mode/*, int *correct_flag, int *separator_flag*/)
{
    //全拼
...
/*!
 * \brief 解析拼音串到音节数组。采用反向最大匹配的方式进行。
 * \param  pin_yin             拼音串（可能包含音节分隔符）
 * \param  *syllable_array     解析后的音节数组，以0为标识
 * \param  syllable_length     音节数组长度
 * \param  fuzzy_mode          模糊音设置
 * \param  pinyin_mode         全拼、双拼
 * \return
 *      音节的数目，等于0的时候一般为错误输入
 */
int ParsePinYinStringReverse(TCHAR *pin_yin, SYLLABLE *syllables, int array_length, int fuzzy_mode, int pinyin_mode/* ,int *correct_flag,int *separator_flag*/)
{
    int i;
 ...
 * \param       str             输出串
 * \param       length          输入串的长度
 * \param       correct_flag    纠错标记
 * \param       tone_char       使用音调(注意不是使用syllable.h中的TONE_CHAR_1等字符，而是将拼音表示为SYLLABLEMAP中的带声调形式，如ā等)
 * \return
 *      字符串长度，0标识没有字符。
 */
int GetSyllableString(SYLLABLE syllable, TCHAR *str, int length, /*int correct_flag, */int tone_char)
{
    ...
    else
    {
        ...
        //for (i = 0; i < default_resource->syllable_map_items; i++)
        for(i = default_resource->syllable_map_items - 1 ;i >=0;i--)
        {
            if (syllable.con != default_resource->syllable_map[i].con ||
                syllable.vow != default_resource->syllable_map[i].vow/* ||
                correct_flag != default_resource->syllable_map[i].correct_flag*/)
                continue;
            ...
            for (j = 0; j <= 3; j++)
            {
                ...
                if (tone_char)
                {
                    ...
                    {
                    case 0:
                        _tcscat_s(str, length, default_resource->syllable_map[i].pin_yin_tone1);
                        break;

                    case 1:
                        _tcscat_s(str, length, default_resource->syllable_map[i].pin_yin_tone2);
                        break;

                    case 2:
                        _tcscat_s(str, length, default_resource->syllable_map[i].pin_yin_tone3);
                        break;

                    case 3:
                        _tcscat_s(str, length, default_resource->syllable_map[i].pin_yin_tone4);
                        break;
                    }
                }
                else
                {
                    _tcscat_s(str, length, default_resource->syllable_map[i].pin_yin);
                    _tcscat_s(str, length, tone_to_string[__1i64 << j]);
                }
            }

            if (!count)
                _tcscpy_s(str, length, default_resource->syllable_map[i].pin_yin);
...
/*!
 * \brief 判断音节首字母是否与参数字母相同。用于单键取词。
 * \param  ch              进行判断的字母
 * \param  syllable        进行判断的音节
 * \return
 *      1：包含，0：不包含
 */
...
/*!
 * \brief 比较一组音节，进行通配符的比较，不进行音调比较。
 *  检查音节可能含有通配符，但被检查音节中不包括这些。
 *  不应有连续两个以上的通配符。
 * \param  syllables                   比较的音节
 * \param  length                      比较音节长度
 * \param  checked_syllables           被比较的音节数组
 * \param  ci                          被比较的词数组
 * \param  checked_length              被比较的词长度
 * \param  fuzzy_mode                  模糊方式
 * \return
 *      相符：1
 *      不相符：0
 */
 ...
 * \param  syllables                   比较的音节
 * \param  length                      比较音节长度
 * \param  checked_syllables           被比较音节
 * \param  checked_length              被比较音节的长度
 * \param  fuzzy_mode                  模糊方式
 * \return
 *      相符：1
 *      不相符：0
...
/*!
 * \brief  比较一组音节，不进行通配符的比较，不进行音调比较。
 *  注：不能有两个以上的通配符。
 * \param  syllables                   比较的音节
 * \param  checked_syllables           被比较音节
 * \param  length                      比较音节长度
...
/*!
 * \brief 比较一组音节的首字母是否与目标相符，进行通配符的比较，
 *  不进行音调比较。
 * \param   letters                     比较的首字母数组
 * \param   length                      比较首字母长度
 * \param   checked_syllables           被比较音节
 * \param   checked_length              被比较音节的长度
 * \return
 *      相符：1
...
/*!
 * \brief 比较一组音节的首字母是否与目标相符，不进行通配符的比较，不进行音调比较。
 * \param  letters                     比较的首字母数组
 * \param  checked_syllables           被比较音节
 * \param  length                      比较首字母长度
 * \return
 ...
 * \param syllable_array
 * \param syllable_count
 */
 ...
 *      xian    -> xi'an
 *      xiang   -> xi'ang
 *  采用方式：元心拼音采用反向最大匹配算法进行音节拆分，现在
 *  需要进行正向的最大匹配；解析后的的结果需要判断是否为完整
 *  拼音。
 *
 * \param  context  PIMCONTEXT 结构
 * \param  syllable_start   音节起始位置
 * \param  syllable_count   音节个数
 * \param  other_syllable_array  正向解析数组指针
 * \param  other_syllable_count  正向解析音节个数
 * \return
 *      存在另一种拆分方式：返回音节数目
 *      不存在：0
 */
int GetOtherSyllableParse(PIMCONTEXT *context, int syllable_start, int syllable_count, SYLLABLE *other_syllables, int other_length)
{
    ...
    int  other_count;
    memset(syllable_string, 0, sizeof(syllable_string));
...
/*!
 * \brief 辅助GetSmallSyllablesParse，完成递归算法
*/
...
/*!
 * \brief 判断是否可以通过拆分单字的音节获得音节的另一种解析方式
 ...
 *
 *  \param syllables               待分析的拼音串
 *  \param syllable_count          syllables中音节的数目
 *  \param syallable_arrays        结果数组，每个元素是一个拼音串
 *  \param other_length            syallable_arrays每个串的音节数目
 *
 *  \return
 *      存在另外的解析方式：    解析方式的数目
 *      不存在：                0
