...
/*!
     * \brief 保存词Cache数据。
     * \param  ci_cache_file_name          词Cache文件名称
     * \return                             成功：1    失败：0
     */
int SaveCiCacheData(const TCHAR *ci_cache_file_name)
{
    if (!default_resource->ci_cache_modified)
        return 1;

    default_resource->ci_cache_modified = 0;
    if (!SaveToFile(ci_cache_file_name, &default_resource->ci_cache, sizeof(default_resource->ci_cache)))
    {
...
{
    if (0 == default_resource->ci_cache.length || CI_CACHE_V66_SIGNATURE == default_resource->ci_cache.signature)
        return;

    if (CI_CACHE_SIGNATURE == default_resource->ci_cache.signature)
    {
        int pos = 0;
        int item_length = (char)default_resource->ci_cache.cache[pos] * sizeof(HZ);

        default_resource->ci_cache.signature = CI_CACHE_V66_SIGNATURE;

        //遍历词的cache
        while(pos + item_length <= default_resource->ci_cache.length)      //需要判定最后一个词条不越界
        {
            char ci_ansi[MAX_WORD_LENGTH * sizeof(HZ)] = {0};
            TCHAR ci_uc[MAX_WORD_LENGTH] = {0};

            memcpy_s(ci_ansi, MAX_WORD_LENGTH * sizeof(HZ), &default_resource->ci_cache.cache[pos + WORDLIB_FEATURE_LENGTH], item_length);

            AnsiToUtf16(ci_ansi, ci_uc, MAX_WORD_LENGTH);

            memcpy_s(&default_resource->ci_cache.cache[pos + WORDLIB_FEATURE_LENGTH], item_length, ci_uc, item_length);

            //下一个词
            pos += WORDLIB_FEATURE_LENGTH + default_resource->ci_cache.cache[pos] * sizeof(HZ);
            item_length = (char)default_resource->ci_cache.cache[pos] * sizeof(HZ);
        }
    }
    else
    {
        default_resource->ci_cache.signature    = CI_CACHE_V66_SIGNATURE;
        default_resource->ci_cache.length       = 0;
        default_resource->ci_cache.max_used_id  = 0;
    }
...
int LoadCiCacheData(const TCHAR *ci_cache_file_name)
{
    ...
    default_resource->ci_cache.length = default_resource->ci_cache.max_used_id = 0;

    length = LoadFromFile(ci_cache_file_name, (char*)&default_resource->ci_cache, sizeof(default_resource->ci_cache));
    if (length < 0)
    {
        default_resource->ci_cache.length = default_resource->ci_cache.max_used_id = 0;
    ...
    default_resource->ci_cache_loaded = 1;
    return 1;
}

/*!
     * \brief 获取最小的使用度
     * \return 最小的使用度的词对应的位置
     */
int GetMinUserCount(){
    int pos = 0;
    int user_count = 0;
    int min_count =CI_MAX_USED_COUNT+1 ;

    //遍历词的cache
    while(pos < default_resource->ci_cache.length)      //需要判定最后一个词条不越界
    {
        user_count = *(int*)&default_resource->ci_cache.cache[pos] >> 8;
...
int FreeCiCacheData()
{
    default_resource->ci_cache_loaded = 0;
    return 1;
}

/*!
     * \brief 在词Cache中找出词的位置。
     * \param  hz          词
     * \param  length      长度
     * \return             -1：没有找到    >0 该词在Cache中的位置（越小越新）
     */
int GetCiCacheInfo(HZ *hz, int length)
{
    ...
    while(pos + item_length <= default_resource->ci_cache.length)      //需要判定最后一个词条不越界
    {
        if ((char)default_resource->ci_cache.cache[pos] == (char)length &&                                     //长度相同
                !memcmp(&default_resource->ci_cache.cache[pos + WORDLIB_FEATURE_LENGTH], hz, length * sizeof(HZ))) //汉字相同
            return pos;
...
/*!
     * \brief 词Cache的使用度到达临界值，需要将所有的词的使用度减少。
     */
void ReduceCiCacheUsedCount()
{
    ...
    //遍历词条
    while(pos < default_resource->ci_cache.length)
    {
        int used_count;
        int length = default_resource->ci_cache.cache[pos];

        if ((int)(pos + WORDLIB_FEATURE_LENGTH + sizeof(HZ) * length) >= default_resource->ci_cache.length)    //越界
            break;

        used_count = *(int*)&default_resource->ci_cache.cache[pos] >> 8;
        used_count -= CI_MAX_USED_COUNT / 2;        //减少一半的值
        *(int*)&default_resource->ci_cache.cache[pos] = (used_count << 8) + length;
...
{
    int pos;                        //Cache中的位置
    int item_length, max_length;               //本词条的长度
    ...
    max_length = MAX_WORD_LENGTH *sizeof(HZ) +WORDLIB_FEATURE_LENGTH;
    ...
    if (pos != -1)      //找到
    {
        used_count = *(int*)&default_resource->ci_cache.cache[pos] >> 8;       //找出词的使用度
        used_count++;
    }
    else        //没有找到的话，当作插入到cache末尾处
    {
        if(default_resource->ci_cache.length + max_length > CI_CACHE_SIZE)
        {
            pos = GetMinUserCount();
        }else{
            pos = default_resource->ci_cache.length;
            default_resource->ci_cache.length     = pos + max_length;
        }
        used_count = 1;
        default_resource->ci_cache.cache[pos] = (char)length;
        for (i = 0; i < (int)(MAX_WORD_LENGTH * sizeof(HZ)); i++)
            if(i < length *sizeof(HZ))
                default_resource->ci_cache.cache[pos + i + WORDLIB_FEATURE_LENGTH] = *((char*)hz + i); //将汉字拷贝到Cache
            else
                default_resource->ci_cache.cache[pos + i + WORDLIB_FEATURE_LENGTH] = 0; //将汉字拷贝到Cache
    }
    ...
    *(int*)&default_resource->ci_cache.cache[pos] = (used_count << 8) + default_resource->ci_cache.cache[pos];    //更新使用度

    //调整顺序
    //(0)(1)....(pos-1)(pos)(pos+1)...(cache_length-1)  ====> (pos)(0)(1)...(pos-1)(pos+1)...(cache_length-1)
    //保存当前词条
    for (i = 0; i < item_length; i++)
        item_save[i] = default_resource->ci_cache.cache[pos + i];

    for (i = pos - 1; i >= 0; i--)
        default_resource->ci_cache.cache[i + item_length] = default_resource->ci_cache.cache[i];

    for (i = 0; i < item_length; i++)
        default_resource->ci_cache.cache[i] = item_save[i];

    //将越界的词Cache长度减少回来
    if (default_resource->ci_cache.length > CI_CACHE_SIZE)
        default_resource->ci_cache.length = CI_CACHE_SIZE;

    //如果使用度越界，则需要将所有词项的使用度减少
    if (used_count > CI_MAX_USED_COUNT)
        ReduceCiCacheUsedCount();

    default_resource->ci_cache_modified = 1;
    return;
}
...
int GetCiUsedCount(int cache_pos)
{
    ...
    return *(int*)&default_resource->ci_cache.cache[cache_pos] >> 8;
}

/*!
 * \brief 比较词中的汉字以及词频，用于处理词的候选顺序。
 *  比较过程：
 *      1. 比较两个词在Cache中的位置
 *      2. 比较两个词的词频
 * \param cand1   用于比较的候选指针
 * \param cand2   用于比较的候选指针
 * \return：
 ...
 * \return      下一条词的指针
 */
void* GetNextCiItem(WORDLIBITEM *item, int iDB)
{
    //feature + syllable + hz
    return (!iDB) ?
                (void*)((char*)item + sizeof(int) + sizeof(SYLLABLE) * USER_MAX_SYLLABLES + sizeof(HZ) * USER_MAX_SYLLABLES) :
                (void*)((char*)item + sizeof(int) + sizeof(SYLLABLE) * item->syllable_length + sizeof(HZ) * item->ci_length);
}

/*!
 * \brief V6B1词库，获得下一条词的指针。
 * \param  item        当前词指针
 * \return 下一条词的指针
 */
...
/*!
 * \brief 获得页中的词候选（没有通配符）。
 * \param  page                    页指针
 * \param  syllable_array          音节序列
 * \param  syllable_count          音节长度
 * \param  candidate_array         候选数组
 * \param  candidate_length        候选数组长度
 * \param  fuzzy_mode              模糊音选项
 * \return 候选词汇数目
 */
int GetCiInPage(PAGE *page, SYLLABLE *syllable_array, int syllable_count, CANDIDATE *candidate_array, int candidate_length, int fuzzy_mode, int iDB)
{
    WORDLIBITEM *item;              //词项
    int count = 0;                  //候选计数
    ...
    //遍历页表
    for (item = (WORDLIBITEM*) page->data; (char*)item < (char*) &page->data + page->data_length; item = GetNextCiItem(item, iDB))
    {
        ...
        candidate_array[count].word.hz        = (iDB) ? GetItemHZPtr(item) : (((USERWORDLIBITEM*) item)->hz);
 ...
 * \return  候选词汇数目
 */
int GetCiInPageWild(PAGE *page, SYLLABLE *syllable_array, int syllable_count, CANDIDATE *candidate_array, int candidate_length, int fuzzy_mode, int iDB)
{
    WORDLIBITEM *item;              //词项
    int count = 0;                  //候选计数
...
/*!
 * \brief 使用首字母，获得页中的词候选（没有通配符）。
 * \param   page                    页指针
 * \param   letters                 音节序列
 * \param   letter_count            音节长度
 * \param   candidate_array         候选数组
 * \param   candidate_length        候选数组长度
 * \param    fuzzy_mode              模糊音选项
 * \return   候选词汇数目
 */
int GetCiInPageByLetter(PAGE *page, const TCHAR *letters, int letter_count, CANDIDATE *candidate_array, int candidate_length, int iDB)
{
    WORDLIBITEM *item;              //词项
    int count = 0;                  //候选计数
 ...
 * \return  候选词汇数目
 */
int GetCiInPageByWildLetter(PAGE *page, const TCHAR *letters, int letter_count, CANDIDATE *candidate_array, int candidate_length, int iDB)
{
    WORDLIBITEM *item;              //词项
    int count = 0;                  //候选计数
 ...
 * \return  候选词汇数目
 */
int GetCiCandidatesByLetter(int wordlib_id, const TCHAR *letters, CANDIDATE *candidate_array, int candidate_length, int iDB)
{
    WORDLIB *wordlib;                           //词库指针
...
/*!
 * \brief 处理词汇被用户选中后的操作：插入到Cache中，增加用户使用度。
 * \param ci_cand         词候选
 */
 ...
 *
 * \param  syllable_array          音节数组
 * \param  syllable_count          音节计数
 * \param  candidate_array         候选数组
 * \param  candidate_count         候选数目
 * \return  剩余候选数目
 */
 ...
 * \param  syllable_array          音节数组
 * \param  syllable_count          音节长度
 * \param  letters                 用户输入串，为0时为不操作
 * \param  candidate_array         候选数组
 * \param  candidate_length        候选数组长度
 * \param  same_ci_syllable_length 是否需要词与音节的长度相同
 * \return  候选数目
 */
int  ProcessCiCandidate(SYLLABLE *syllable_array, int syllable_count, const TCHAR *letters, CANDIDATE *candidate_array, int candidate_length, int same_ci_syllable_length)
{
            ...
            (int)_tcslen(letters) >= pim_config->first_letter_input_min_hz)
    {
        ...
        while (wordlib_id != -1)
        {
            count += GetCiCandidatesByLetter(wordlib_id, letters, candidate_array + count, candidate_length - count, wordlib_id);
            wordlib_id = GetNextWordLibId(wordlib_id);
        };
    }
...
/*!
 * \brief 获得用户词库中的词汇候选，用于ICW的用户词读取。
 * \param  syllable_array          音节序列
 * \param  syllable_count          音节长度
 * \param  candidate_array         候选数组
 * \param  candidate_length        候选数组长度
...
int GetUserCiCandidates(SYLLABLE *syllable_array, int syllable_count, CANDIDATE *candidate_array, int candidate_length, int fuzzy_mode)
{
    ...
    if (!candidate_length || syllable_count < 2 || syllable_count > USER_MAX_SYLLABLES)
        return 0;
    ...
    for (i = 0; i < count; i++)
    {
        candidate_array[i].word.source = 0;
        candidate_array[i].type        = CAND_TYPE_CI;
    }
...
int AddCi(SYLLABLE *syllable, int syllable_length, HZ *hz, int hz_length)
{
    ...
    //UCS4的词不加入用户词库
    if (syllable_length != hz_length || syllable_length > USER_MAX_SYLLABLES)
        return 0;

    if (!AddCiToUserWordLibrary(hz, hz_length, syllable, syllable_length, USER_BASE_FREQ))
        return 0;       //失败
...
//  int index = 0;
//
//  while(index < default_resource->new_ci.length)
//  {
//      if (default_resource->new_ci.data[index] == (char)ci_length && default_resource->new_ci.data[index +1] == (char)syllable_length &&
//          !memcmp(&default_resource->new_ci.data[index + 2], ci_str, ci_length * sizeof(HZ)) &&
//          !memcmp(&default_resource->new_ci.data[index + 2 + ci_length * sizeof(HZ)], syllable, syllable_length * sizeof(SYLLABLE)))
//          return index;
//
//      index += 2 + default_resource->new_ci.data[index] * sizeof(HZ) + default_resource->new_ci.data[index + 1] * sizeof(SYLLABLE);
//  }
//
//  return -1;
//}
...
//  for (i = default_resource->new_ci.length - 1; i >= 0; i--)
//      default_resource->new_ci.data[i + size] = default_resource->new_ci.data[i];
//
//  default_resource->new_ci.data[0] = (char)ci_length;
//  default_resource->new_ci.data[1] = (char)syllable_length;
//
//  memcpy(&default_resource->new_ci.data[2], ci_str, ci_length * sizeof(HZ));
//  memcpy(&default_resource->new_ci.data[2 + ci_length * sizeof(HZ)], syllable, syllable_length * sizeof(SYLLABLE));
//
//  default_resource->new_ci.length += size;
//  if (default_resource->new_ci.length > sizeof(default_resource->new_ci.data))
//      default_resource->new_ci.length = sizeof(default_resource->new_ci.data);
//
//  default_resource->new_ci_modified = 1;
//  return 1;
//}
...
//
//  //依次向前移动一个词条的位置
//  for (i = index; i < default_resource->new_ci.length - size; i++)
//      default_resource->new_ci.data[i] = default_resource->new_ci.data[i + size];
//
//  default_resource->new_ci.length -= size;
//  if (default_resource->new_ci.length < 0)
//      default_resource->new_ci.length = 0;
//
//  default_resource->new_ci_modified = 1;
//}
...
//  char tmp[0x100];
//
//  ci_length       = default_resource->new_ci.data[index];
//  syllable_length = default_resource->new_ci.data[index + 1];
//  size            = ci_length * sizeof(HZ) + syllable_length * sizeof(SYLLABLE) + 2;
//
//  if (buffer_length - 1 < size || sizeof(tmp) - 1 < size)
//      return -1;
//
//  memcpy(tmp, &(default_resource->new_ci.data[index]), size);
//  tmp[size] = 0;
//
//  //存储词条长度以及音节长度
...
//  while(index < default_resource->new_ci.length)
//  {
//      if (default_resource->new_ci.data[index + 2] != 0)     //不是被删除的
//      {
...
/*!
 * \brief 检查是否在用户词库中存在这个词，如果不存在，则加入到新词表中
 */
...
//  TCHAR name[MAX_PATH];
//
//  if (default_resource->new_ci_loaded)
//      return 1;
//
...
//  default_resource->new_ci.length = 0;
//
//  length = LoadFromFile(name, (char*)&default_resource->new_ci, sizeof(default_resource->new_ci));
//  if (length < 0)
//  {
...
//  }
//
//  default_resource->new_ci_loaded = 1;
//  return 1;
//}
...
//  TCHAR name[MAX_PATH];
//
//  if (!default_resource->new_ci_modified)
//      return 1;
//
//  GetFileFullName(TYPE_ALLAPP, NEWCI_FILE_NAME, name);
//  if (!SaveToFile(name, (char*)&default_resource->new_ci, sizeof(default_resource->new_ci)))
//  {
//      Log(LOG_ID, L"写入新词表文件失败,file:%s", name);
//      return 0;
//  }
//
//  default_resource->new_ci_modified = 0;
//  return 1;
//}
...
//{
//  SaveNewWordTable();
//  default_resource->new_ci_loaded = 0;
//}
